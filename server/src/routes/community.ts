...
communityRouter.post("/user-suggestions", async (c) => {
  try {
    const body = await c.req.json();
    const token = body.token;
    ...
    const otherUsersInSameCommunities =
      await prisma.communityMembership.findMany({
        where: {
          communityId: {
            in: userCommunities.map(({ community }) => community.id),
          },
          NOT: { userId: user.id },
        },
        select: {
    ...
    const suggestedCommunities = await prisma.communityMembership.findMany({
      where: {
        userId: { in: otherUsersInSameCommunities.map(({ user }) => user.id) },
        community: {
          NOT: {
            id: { in: userCommunities.map(({ community }) => community.id) },
          },
        },
      },
      select: {
    ...
    const suggestedUsers = await prisma.following.findMany({
      where: {
        followingId: {
          in: followingUsers.map(({ following }) => following.id),
        },
    ...
    if (suggestedCommunities.length === 0 && suggestedUsers.length === 0) {
      ...
      return c.json({
        suggestedCommunities: randomCommunities,
    ...
    return c.json({
      ...
      suggestedUsers: suggestedUsers.map(({ follower }) => follower),
    });
  } catch (error) {
    return c.json({ status: 500, message: "Network error" });
  }
